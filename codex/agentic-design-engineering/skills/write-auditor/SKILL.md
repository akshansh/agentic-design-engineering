---
name: write-auditor
description: "Evaluates UI copy against the Write framework's seven principles: partnership, simplification, purpose, invitation, connection, compassion, metaphor language. Returns scored findings with violation details."
---
...
- The Style metaphor (if established) — needed for principle 7 evaluation
...
- If Style metaphor exists: does copy inhabit the metaphor?
...
- If NO Style metaphor exists: score based on whether copy has any distinctive character (not generic AI default)
...
```json
{
  ...
  "summary": "Copy is functional but generic. Error messages blame users. No metaphor language despite Style atmosphere being established.",
  "readyForShip": false
}
```
...
- If no Style metaphor exists, principle 7 is scored on whether copy has ANY distinctive character — not penalized for lacking a metaphor that was never established.
