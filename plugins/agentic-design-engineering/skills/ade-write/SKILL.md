...
# /ade:write — Communication Style Pass
...
## Baseline: Run Write Auditor

Before rewriting anything, run the `write-auditor` agent on the existing code to establish baseline scores. Record the scores — you'll compare against these after all rewrites.
...
### Principle 7: Metaphor Language (If Style Is Established)

If the page has a physical metaphor from Style, copy should speak its language.
...
- Copy that Style has already transformed to metaphor language (don't override)
...
> Copy now sounds intentional. Your interface works (Build), feels like somewhere (Style), is alive (Move), and speaks with a human voice (Write). Run `/ade:audit` for a final combined score.

---

## What NOT to Do

- Do not change the UI structure — that's Build
- Do not change the atmosphere — that's Style
- Do not add interactivity — that's Move
