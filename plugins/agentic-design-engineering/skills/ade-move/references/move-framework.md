...
*Companion to Build (structure) and Style (atmosphere) — this framework governs engagement.*
...
**Ask:** Which kinds of fun does your product support? Most products only support Challenge (task completion) and maybe Discovery. Adding Sensation (Style atmosphere), Fantasy (metaphor), and Submission (pleasant browsing) transforms the experience.

---

## The Move Transformation Process
...
For each dead spot, apply the Move dimension that's most needed:
...
Move score: ___/50 (10 per dimension, aim for 35+)
```
...
## Relationship to Build and Style

The three frameworks form a complete design system:

```
Build — Does it work?        Structure, accessibility, hierarchy
Style — Does it feel?        Atmosphere, metaphor, texture
Move — Does it engage?      Feedback, discovery, agency, physics
```

**Apply in this order:**
1. **Build first** — Fix structure and accessibility. A playful interface that's inaccessible is exclusionary.
2. **Style second** — Build the atmosphere. An engaging interface without atmosphere is a game in a white room.
3. **Move last** — Add interactivity and play. Engagement on top of structure and atmosphere creates an experience that works, feels, and captivates.
...
**Build ensures people can use it. Style ensures they want to be there. Move ensures they want to come back.**
...
- **Accessibility-first contexts** — Screen reader users, motor impairments. All Move features must degrade gracefully and respect user preferences.
...
- **Very early MVPs** — Get the structure right (Build) first. Play without structure is chaos.
- **When the user is stressed or urgent** — Error recovery, payment flows, deadline-driven tools. Be helpful, not playful.

Move is for products where **engagement is a differentiator** — where users have a choice of tools and the one that feels alive wins their attention, loyalty, and advocacy.
...
**Companion to:** Build Framework Ultimate, Style Framework
